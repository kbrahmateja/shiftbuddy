...
// ─────────────────────────────────────────────────────────────
// Shows browser-detected timezone + lets user override and save
// preference to localStorage. Syncs across tabs via storage event.
// ─────────────────────────────────────────────────────────────
...
interface Props {
  sessionTimezone: string; // from server session (profile default)
}
...
export function TimezoneSettings({ sessionTimezone }: Props) {
  ...
  const [saved_flash, setSavedFlash] = useState(false);
    ...
    <div className="space-y-1">
      ...
      {!editing ? (
        <div className="flex items-center gap-2">
          <div className="flex-1 flex items-center gap-2 rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700">
            <Globe className="h-3.5 w-3.5 text-gray-400 shrink-0" />
            <span className="flex-1">{effectiveLabel}</span>
            {isOverridden ? (
          ...
          <button
            ...
            className="rounded-md border border-gray-200 bg-white px-3 py-2 text-xs text-gray-600 hover:bg-gray-50 transition-colors"
          >
            Change
          </button>
        ...
        <div className="space-y-2">
          ...
          {!isOverridden && (
            <p className="text-[11px] text-gray-400">
              Auto-detected: <strong>{detectedLabel}</strong>. Change only if incorrect.
            </p>
          )}
        </div>
      )}
