...
import { HandoverAttendanceSheet, type AttendanceData } from "./HandoverAttendanceSheet";
...
export function CompileHandoverClient({ user }: Props) {
  ...
  const quorumMet = attendance?.quorumMet ?? false;
    ...
    <div className="p-6">
      <div className="mb-5">
        ...
        <p className="mt-0.5 text-sm text-gray-500">
          Mark attendance for both shifts, review team updates, then submit the handover summary.
        </p>
      </div>

      <div className="space-y-6">

        {/* ── Step 1: Attendance Sheet ───────────────────────────── */}
        <div className="rounded-lg border bg-white p-5">
          <HandoverAttendanceSheet onChange={setAttendance} />
        </div>

        {/* ── Step 2: Team updates + compile form ───────────────── */}
            ...
            <div className="rounded-lg border bg-white sticky top-4">
              <div className="flex items-center justify-between px-4 py-3 border-b">
                ...
                {submissions.length > 0 && (
                  <button type="button" onClick={autoFill} className="text-xs text-indigo-600 hover:underline">
                    Auto-fill →
                  </button>
                )}
              </div>
              ...
              {submissions.length === 0 ? (
                <div className="p-6 text-center text-xs text-gray-400">No member submissions yet.</div>
              ) : (
          ...
          <div className="lg:col-span-2">
            <form onSubmit={handleSubmit} className="rounded-lg border bg-white p-6 space-y-5">
              ...
              <div className="rounded-md bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-800">
                <strong>30-minute SLA:</strong> The incoming lead must acknowledge this handover within 30 minutes.
              </div>
              ...
              {!quorumMet && (
                <div className="flex items-start gap-2 rounded-md bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700">
                  ...
                  <span>
                    Cannot submit — attendance quorum not met. Mark at least one Lead as Present from each shift above.
                  </span>
                </div>
              )}
              ...
              <div className="flex gap-3 pt-1">
                <button
                  type="submit"
                  disabled={!quorumMet}
                  className="flex-1 rounded-md bg-indigo-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed transition-opacity"
                >
                  Submit Handover
                </button>
                <Link
